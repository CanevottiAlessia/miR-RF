...
out_file1= out_folder / f"{user_file.name}_temp_SNPpot_SEQinsterted_fasta.txt"
...
rnafold_output_file = out_folder / f"{user_file2.name}_potential_RNAfold.txt"

#user_file2 + '_potential_RNAfold.txt'  # Define where RNAfold output will be saved
try:
    # Construct the RNAfold command
    command = ["RNAfold", "-p", "-d2", "--noLP", "--noDP", "--noPS", "--jobs=1", out_file1]
