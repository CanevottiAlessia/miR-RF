...
if __name__=="__main__":
   user_file = sys.argv[1]      # classificazione originale
   user_file2 = sys.argv[2]     # file con la nuova classificazione
   out_folder = sys.argv[3]

user_file=Path(user_file)
user_file2=Path(user_file2)
out_folder = Path(out_folder)

# BASE

#file_path = user_file 
df1 = pd.read_csv(user_file, sep='\t')
df1 = df1[['miRNA name', 'prediction']]

# POTENTIAL

#file_path = user_file2 
df = pd.read_csv(user_file2, sep='\t')
...
out_file_name= out_folder / f"{user_file.name}_temp_classes_table.tsv"
merging_df.to_csv(out_file_name, sep="\t", index=False) #merging_df.to_csv("temp_classes_table.tsv", sep="\t", index=False)
